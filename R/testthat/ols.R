test_that("unregularized gaussian models reduces to OLS, with no intercept", {
  ...
  sgs = fit_sgs(X=X,y=y, groups=groups, type="linear", lambda=0, alpha=1, FDR=0.1, gFDR=0.1, intercept=FALSE, standardise="none")
...
test_that("unregularized gaussian models reduces to OLS, with intercept", {
  ...
  sgs = fit_sgs(X=X,y=y, groups=groups, type="linear", lambda=0, alpha=1, FDR=0.1, gFDR=0.1, standardise="none", intercept=TRUE)
